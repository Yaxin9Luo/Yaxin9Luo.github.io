...
import {TIME_MODES,TIME_PERIODS,normalizeTimeMode,formatClockTime,periodForPhase} from './time-contract.js';
...
const copy={
  ...
  gameplay:['Optional play','可选玩法'],timeOfDay:['Time of day','时段'],auto:['Auto cycle','自动循环'],dawn:['Dawn','清晨'],day:['Day','白天'],dusk:['Dusk','傍晚'],night:['Night','夜晚'],musicVolume:['Music volume','音乐音量'],effectsVolume:['World sounds','环境与动作音量'],
  ...
  primary:['View portfolio','查看个人资料'],explore:['Explore the academy','探索学院'],papers:['Publications','学术论文'],projects:['Projects','研究作品'],cv:['Download CV','下载简历'],garden:['Yuanmingyuan','圆明园'],
  ...
  detailLoading:['Scene details are loading…','场景细节加载中…'],detailReady:['Full scene detail ready','完整场景细节已就绪'],detailReload:['Reload page','重新加载页面'],detailDegraded:['Some details could not load. Exploration and reading remain available.','部分细节未能加载，仍可探索和阅读。'],
  coreTimeout:['The 3D scene took more than 20 seconds. You can keep reading or try again.','3D 场景加载超过 20 秒。你可以继续阅读资料，或重新尝试。'],coreResourceError:['A scene resource could not load. You can keep reading or try again.','有场景资源未能加载。你可以继续阅读资料，或重新尝试。'],
  ...
  controlTitle:['On foot, or above the rooftops.','漫步学院，或飞越屋顶。'],controlIntro:['Explore at your own pace, or open the map and teleport anywhere.','按自己的节奏探索，也可以打开地图瞬移至任意地点。'],
  ctrlFlight:['Walk / fly','步行 / 飞行'],ctrlAltitude:['Rise / descend','上升 / 下降'],ctrlSpell:['Cast selected spell','释放当前咒语'],ctrlChoose:['Choose a spell','选择咒语'],ctrlShield:['Protego shield','盔甲护身'],ctrlInteract:['Read nearby chapter','阅读附近地点的资料'],ctrlMenu:['Open / close the grimoire','打开 / 关闭手记'],ctrlMap:['Open the map','打开地图'],
  mobileHelp:['Use the thumbstick to walk or fly. Summon your broom before using altitude controls. Wand light and safe landing are also available as buttons.','用摇杆步行或飞行；先召唤扫帚，再使用升降控制。魔杖照明与安全落地也有触屏按钮。'],
  breathe:['Take a breath. Your world can wait.','歇一会儿，世界会等你。'],paused:['Paused','已暂停'],resume:['Resume exploring','继续探索'],
  worldHUD:['Exploration controls','探索控制'],clockSettings:['World clock · time settings','世界时钟 · 时段设置'],noon:['Noon','正午'],midnight:['Late night','深夜'],fixed:['Fixed','固定'],clockAuto:['Auto · 4 min / day','自动 · 4 分钟一天'],clockPaused:['Paused','暂停'],clockReading:['Paused · reading','暂停 · 阅读中'],clockBackground:['Paused · away','暂停 · 已离开'],clockGraphics:['Paused · graphics recovery','暂停 · 画面恢复中'],clockMotion:['Paused · reduced motion','暂停 · 减少动态'],clockTransition:['Changing time','正在切换时段'],clockJump:['Jump to the next period · continue','跳到下一时段 · 继续循环'],timeHelp:['One day takes four active minutes. Reading, background tabs and reduced motion pause the clock. Fixed periods stay still.','一天为四分钟活动时间。阅读、离开页面及减少动态会暂停时钟；固定时段保持不变。'],jumpContinue:['Jump here, then continue the day','跳到此时段，然后继续循环'],illumination:['Wand light','魔杖照明'],lightOn:['On','已开启'],lightOff:['Off','已关闭'],summonBroom:['Summon broom','召唤扫帚'],landBroom:['Land safely','安全落地'],broomBusy:['Changing stance','正在转换姿态'],ctrlBroom:['Summon broom / land safely','召唤扫帚 / 安全落地'],move:['Move','移动'],
...
export class Interface {
  ...
  render(){const t=this.t;
    this.root.innerHTML=`<main class="experience" id="experience">
      ...
      <div class="game-interface" hidden><button class="optional-play-toggle" data-action="gameplay" aria-pressed="${this.options.gameplay}">${icon('spark')}<span data-i18n="gameplay">${t('gameplay')}</span></button>
        ...
        <section class="world-hud" aria-label="${t('worldHUD')}" data-locomotion="grounded">
  ...
  updateStatus(){
    ...
    const detail=this.root.querySelector('.world-detail-status');
    if(detail){detail.hidden=!this.ready;detail.textContent=this.t(state.enhancements==='ready'?'detailReady':state.enhancements==='degraded'?'detailDegraded':'detailLoading');}
    const reload=this.root.querySelector('[data-action="world-reload"]');
    if(reload){reload.hidden=!(this.ready&&state.availability==='interactive'&&state.enhancements==='degraded');reload.textContent=this.t('detailReload');}
    const failureKey=state.error?.type==='cancelled'?'staticMode':state.error?.type==='timeout'?'coreTimeout':['http','network','parse'].includes(state.error?.type)?'coreResourceError':'error';
  ...
  action(action,id){
    ...
    if(action==='world-reload'){if(this.ready&&this.loadingState?.availability==='interactive'&&this.loadingState.enhancements==='degraded')location.reload();return;}
    ...
    if(action==='illumination'||action==='broom'){if(this.view||this.exhibition||this.lightboxOpen)return;this.game?.[action==='illumination'?'toggleIllumination':'toggleBroom']?.();return;}
    if(action==='time-jump'){if(!TIME_PERIODS.includes(id)||!this.game?.started)return;this.close({history:'replace',all:true});this.game.jumpToTime?.(id);return;}
  ...
  applyLanguage(){
    ...
    this.root.querySelector('.rail-traditional').innerHTML=`${t('traditional')}${icon('exit')}`;this.root.querySelector('.controls-hint').innerHTML=`<span><kbd>W A S D</kbd> ${t('move')}</span><span><kbd>SHIFT</kbd> ${t('boost')}</span><span><kbd>SPACE</kbd> ${t('cast')}</span><button data-action="controls" aria-label="${t('controls')}">${icon('help')}</button>`;
  ...
  updateWorldHUD(s=this.snapshot,{force=false,now=performance.now()}={}){
    const t=this.t,environment=s.environment,mode=typeof s.locomotion==='string'?s.locomotion:s.locomotion?.mode||s.locomotion?.state||'grounded';
    const flying=['flying','flight','boost'].includes(mode),grounded=['grounded','ground','idle','walk','walking','run','running'].includes(mode),busy=!flying&&!grounded;
    const active=Boolean(s.started)&&!s.paused&&!this.view&&!this.exhibition&&!this.lightboxOpen;
    const text=(selector,value)=>{const el=this.root.querySelector(selector);if(el&&el.textContent!==value)el.textContent=value;};
    const attr=(el,name,value)=>{if(el.getAttribute(name)!==String(value))el.setAttribute(name,String(value));};
    const hud=this.root.querySelector('.world-hud');if(hud){hud.dataset.locomotion=flying?'flying':busy?'transition':'grounded';attr(hud,'aria-label',t('worldHUD'));}
    const light=this.root.querySelector('[data-action="illumination"]'),broom=this.root.querySelector('[data-action="broom"]');
    if(light){light.disabled=!active||s.illumination?.available===false;attr(light,'aria-pressed',Boolean(s.illumination?.enabled));attr(light,'aria-label',`${t('illumination')} · ${t(s.illumination?.enabled?'lightOn':'lightOff')}`);}
    if(broom){broom.disabled=!active||busy;attr(broom,'aria-label',t(busy?'broomBusy':flying?'landBroom':'summonBroom'));}
    text('[data-hud-label="illumination"]',t('illumination'));text('[data-hud-label="broom"]',t(busy?'broomBusy':flying?'landBroom':'summonBroom'));
    this.root.querySelectorAll('[data-control="up"],[data-control="down"],[data-action="altitude"]').forEach(button=>button.disabled=!active||!flying);
    const clock=this.root.querySelector('.world-clock');if(!clock)return;clock.hidden=!environment;attr(clock,'aria-label',t('clockSettings'));
    if(!environment)return;
    const fixed=environment.mode!=='auto',reason=environment.pauseReason,period=environment.period||periodForPhase(environment.phase);
    const state=fixed?'fixed':environment.clockState==='paused'||s.paused?'paused':'auto';
    const stateKey=[this.options.lang,environment.mode,state,reason,period,Boolean(environment.transitioning)].join('/');
    if(!force&&stateKey===this._clockStateKey&&now-(this._clockPaint??-Infinity)<250)return;
    this._clockPaint=now;this._clockStateKey=stateKey;
    const status=fixed?`${t('fixed')} · ${t(environment.mode)}`:state==='paused'?t(reason==='reduced-motion'?'clockMotion':['background','hidden'].includes(reason)?'clockBackground':['context-lost','graphics'].includes(reason)?'clockGraphics':reason==='reading'||reason==='exhibition'?'clockReading':'clockPaused'):t('clockAuto');
    text('#world-clock-time',environment.clockText||formatClockTime(environment.phase));text('#world-clock-period',t(period));text('#world-clock-state',environment.transitioning?`${status} · ${t('clockTransition')}`:status);
    clock.dataset.state=state;
  }
  update(s){
    this.snapshot=s;this.syncStarted(s.started);this.updateWorldHUD(s);
    ...
    const nearby=locations.find(l=>l.id===s.nearest),paper=publications.find(p=>p.id===s.nearestPaper),exhibit=getProject(s.nearestExhibition),companion=s.started&&!s.paused?s.nearestCompanion:null;
    this.root.querySelector('.interaction-prompt').hidden=!!this.exhibition||(!nearby&&!paper&&!exhibit&&!s.nearestClock&&!s.nearestArtifact&&!companion);
    ...
    else if(companion)this.root.querySelector('#nearby-name').textContent=companion.label[this.options.lang];
    this.root.querySelector('.interaction-prompt strong').textContent=!exhibit&&s.nearestClock?this.t('clockJump'):!exhibit&&!s.nearestClock&&!s.nearestArtifact&&!paper&&!nearby&&companion?companion.actionLabel[this.options.lang]:this.t('interact');
  ...
  controlsContent(){const t=this.t;const rows=[['W A S D / ↑ ← ↓ →','ctrlFlight'],['SHIFT','boost'],['R / F','ctrlAltitude'],['V','ctrlCamera'],['SPACE','ctrlSpell'],['1 / 2 / 3','ctrlChoose'],['Q','ctrlShield'],['L','illumination'],['B','ctrlBroom'],['E','ctrlInteract'],['ESC','ctrlMenu'],['M','ctrlMap']];return `<div class="extra-page"><span class="page-eyebrow">${t('controls')}</span><h2>${t('controlTitle')}</h2><p class="page-lede">${t('controlIntro')}</p><div class="control-grid">${rows.map(([k,v])=>`<div><span>${t(v)}</span><kbd>${k}</kbd></div>`).join('')}</div><p class="control-note">${icon('compass')}${t('rotate')}</p><p class="mobile-note">${t('mobileHelp')}</p><button class="paper-button filled" data-action="close">${t('resume')}${icon('arrow')}</button></div>`;}
}
