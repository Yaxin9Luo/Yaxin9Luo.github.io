import {locations,spellDefinitions,crystalPositions,ringPositions} from './locations.js';
...
export class Interface {
  constructor(root){
    ...
    this.game=null;this.view=null;this.lastSection='about';this.lastFocus=null;this.ready=false;this.failed=false;this.resetConfirm=false;this.snapshot={progress:freshProgress(),mana:100,health:100,spell:0,position:{x:9,y:8,z:48},race:null};
  ...
  action(action,id){
    ...
    if(action==='interact'&&this.snapshot.nearest)this.open(this.snapshot.nearest);
  ...
  open(view){
    ...
    this.view=view;if(locations.some(l=>l.id===view))this.lastSection=view;
  ...
  toggleLanguage(){this.options.lang=this.options.lang==='en'?'zh':'en';this.persist();this.applyLanguage();const current=this.view;if(current)this.open(current);}
  applyLanguage(){
    ...
    this.root.querySelector('.touch-boost').textContent=t('boost');this.root.querySelector('[data-control="up"]').setAttribute('aria-label',t('ascend'));this.root.querySelector('[data-control="down"]').setAttribute('aria-label',t('descend'));this.root.querySelector('[data-control="fire"]').setAttribute('aria-label',t('cast'));
  ...
  update(s){
    ...
    const nearby=locations.find(l=>l.id===s.nearest);this.root.querySelector('.interaction-prompt').hidden=!nearby;if(nearby)this.root.querySelector('#nearby-name').textContent=nearby.name[this.options.lang];
    this.root.querySelectorAll('.player-dot,.player-halo').forEach(p=>{p.setAttribute('cx',s.position.x+90);p.setAttribute('cy',s.position.z+85);});
    const race=s.race?.active;this.root.querySelector('.race-hud').hidden=!race;if(race){this.root.querySelector('#race-progress').textContent=`${s.race.index} / ${ringPositions.length}`;this.root.querySelector('#race-time').textContent=Math.max(0,s.race.timeLeft).toFixed(1);}
  ...
  mapSVG(small=false){return `<svg class="world-map ${small?'small':''}" viewBox="0 0 180 180" role="img" aria-label="${this.t('map')}"><defs><pattern id="${small?'mini':'full'}grid" width="12" height="12" patternUnits="userSpaceOnUse"><path d="M12 0H0v12" fill="none" stroke="currentColor" stroke-width=".2" opacity=".15"/></pattern></defs><rect width="180" height="180" fill="url(#${small?'mini':'full'}grid)"/><path class="map-waterline" d="M21 36 34 21 58 14 72 26 93 37 117 20 144 20 164 37 161 57 166 86 167 113 155 143 130 166 108 164 81 167 61 161 40 171 20 149 10 123 17 98 15 74 27 57Z"/><path class="map-land" d="M26 31 41 21 57 24 66 38 69 52 78 53 95 39 114 52 119 33 135 21 152 29 158 44 146 61 159 82 158 107 143 133 125 151 97 151 81 156 61 146 40 157 27 141 21 115 24 91 26 76 43 64 39 53 25 49Z"/><path class="map-paths" d="M90 106Q70 90 46 97M90 106Q120 105 130 122M90 106V81M90 106Q66 135 61 141M46 97 42 54M90 81 133 55"/>${locations.map(l=>`<g transform="translate(${l.x+90} ${l.z+85})"><circle r="${small?'3':'6'}"/><text y="${small?'-6':'1.9'}" text-anchor="middle">${l.number}</text></g>`).join('')}<circle class="player-halo" cx="99" cy="133" r="5"/><circle class="player-dot" cx="99" cy="133" r="2.6"/><text class="map-north" x="165" y="16">N</text><path d="m167 20-2 7 2-2 2 2Z" fill="currentColor"/></svg>`;}
  mapContent(){const t=this.t;return `<div class="extra-page map-page"><span class="page-eyebrow">${t('allOpen')}</span><h2>${t('mapTitle')}</h2><p class="page-lede">${t('mapIntro')}</p><div class="map-layout"><div class="large-map">${this.mapSVG()}<span class="map-legend"><i></i>${t('mapYou')}</span><span class="map-caption">${this.options.lang==='zh'?'学院全景地图<br>从好奇心出发':'THE ACADEMY GROUNDS<br>EST. IN CURIOSITY'}</span></div><div class="destination-list">${locations.map(l=>`<div class="destination-row"><span class="destination-symbol">${icon(l.icon)}</span><div><h3>${l.name[this.options.lang]}</h3><p>${l.subtitle[this.options.lang]}</p><div><button data-action="section" data-id="${l.id}">${t('read')}${icon('arrow')}</button><button data-action="travel" data-id="${l.id}">${icon('portal')}${t('teleport')}</button></div></div></div>`).join('')}</div></div></div>`;}
