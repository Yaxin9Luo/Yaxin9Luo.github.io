...
import {applyGroundChroma} from './ground-chroma.js';
import {bindRootedLoamMaterial} from './rooted-loam.js';
import { mergeGeometries } from 'three/addons/utils/BufferGeometryUtils.js';
import { Water } from 'three/addons/objects/Water.js';
import {createWaterReflectionController} from './water-reflection.js';
import { HDRLoader } from 'three/addons/loaders/HDRLoader.js';
import {herbariumAt,herbariumLawnAt,herbariumSoilAt,herbariumCommunitySoilAt,subscribeHerbariumCommunitySoil,herbariumArrivalReserved} from './herbarium-layout.js';
...
import {partitionPlantBatch} from './plant-batches.js';
import {createGrassTuftVariants} from './grass-geometry.js';
import {createGrassDistribution} from './grass-distribution.js';
...
function plantingMaterialMap(){
  ...
  const refresh=()=>{for(let z=0;z<size;z++)for(let x=0;x<size;x++){
  ...
  if(communityTexture)communityTexture.needsUpdate=true;};refresh();
  communityTexture=new THREE.DataTexture(data,size,size);communityTexture.minFilter=communityTexture.magFilter=THREE.LinearFilter;communityTexture.colorSpace=THREE.NoColorSpace;communityTexture.needsUpdate=true;communityTexture.userData.sharedAsset=true;
  const texture=communityTexture,unsubscribe=subscribeHerbariumCommunitySoil(refresh);
  texture.addEventListener('dispose',()=>{unsubscribe();if(communityTexture===texture)communityTexture=undefined;});
...
export function groundMaterial({transition=false}={}) {
  ...
  const applyRootedLoam=bindRootedLoamMaterial(m);
  const bindings=[];
  ...
  m.onBeforeCompile=shader=>{
    ...
    shader.fragmentShader=shader.fragmentShader.replace('#include <map_fragment>',`
      ...
      vec3 sourceHumusColor=continuousSoil(humusMap,soilUv).rgb;
      vec3 humusColor=sourceHumusColor;
    ...
    shader.fragmentShader=shader.fragmentShader.replace('#include <roughnessmap_fragment>',`float roughnessFactor=roughness;
      ...
      #endif`);
    applyGroundChroma(shader);
    applyRootedLoam(shader);
  };
  m.customProgramCacheKey=()=> `terrain-translated-community-pbr-v8-${transition}-local-chroma-rooted-loam-v1`; return m;
}
...
export function createLake(root, scene) {
  ...
  root.add(water);
  const reflectionController=createWaterReflectionController(water);
  // Game disposes scene materials, but uniform-only textures/targets need an owner.
  let disposed=false;
  water.material.addEventListener('dispose',()=>{if(disposed)return;disposed=true;reflectionController.dispose();normals.dispose();shoreMap.dispose();reflection.renderTarget.dispose();});
  if(environment&&scene){scene.environment=environment;scene.environmentIntensity=.14;}
  return {update(time,reduced){water.material.uniforms.time.value=reduced?0:time*.18;wind.value=reduced?0:time;updateGroveWind(time,reduced);},water,reflection:reflectionController};
}
...
function createGrassMaterial(){const material=leafMaterial(null,'#d3dac1');material.vertexColors=true;material.alphaTest=0;return material;}

/** Native-scale studio source: exactly the same three complete field tufts. */
export function createGrassSpecimen(){
  const group=new THREE.Group(),material=createGrassMaterial(),resources=new Set([material]);group.name='Fine folded meadow grass · three original tufts';
  createGrassTuftVariants().forEach((geometry,variant)=>{
    const mesh=new THREE.Mesh(geometry,material);mesh.name=`Grass tuft variant ${variant+1}`;mesh.position.set((variant-1)*.55,-.025,0);mesh.receiveShadow=true;mesh.userData.grassVariant=variant;attachWindShadows(mesh);
    mesh.boundingBox=geometry.boundingBox.clone().expandByScalar(.18);mesh.boundingSphere=geometry.boundingSphere.clone();mesh.boundingSphere.radius+=.18;
    resources.add(geometry);resources.add(mesh.customDepthMaterial);resources.add(mesh.customDistanceMaterial);group.add(mesh);
  });
  group.updateMatrixWorld(true);const box=new THREE.Box3().setFromObject(group,true),size=box.getSize(new THREE.Vector3()),center=box.getCenter(new THREE.Vector3()),span=Math.max(size.x,size.y,size.z),target=center.toArray();
  group.userData={version:9,studyOwner:'grass',studyFloorY:0,actualBounds:{min:box.min.toArray(),max:box.max.toArray()},dimensions:{width:size.x,height:size.y,depth:size.z},maxWindDisplacement:.18,
    studyViews:{front:{eye:[center.x,.68,center.z+span*1.65],target},threequarter:{eye:[center.x+span*1.2,.88,center.z+span*1.45],target},profile:{eye:[center.x+span*1.65,.65,center.z],target},back:{eye:[center.x,.68,center.z-span*1.65],target},overview:{eye:[center.x+.4,span*1.7,center.z+.8],target},close:{eye:[.42,.40,.78],target:[0,.24,0]},roots:{eye:[.15,.13,.38],target:[0,.025,0]},eye:{eye:[.55,.50,1.10],target:[0,.20,0]},interior:{eye:[.15,.20,.48],target:[0,.13,0]}}};
  let disposed=false;group.dispose=()=>{if(disposed)return;disposed=true;group.removeFromParent();for(const resource of resources)resource.dispose();group.userData.disposed=true;};return group;
}
...
export function createVegetation(root,heightAt,nearPath=()=>false,{lod=false,trees=true}={}) {
  ...
  const dummy=new THREE.Object3D(),plantBatches=[];
  const batch=(geo,mat,placements,name,shadow=true,spatial=true)=>{
    ...
    mesh.castShadow=shadow;mesh.receiveShadow=true;attachWindShadows(mesh);mesh.computeBoundingSphere();root.add(mesh);if(spatial)plantBatches.push(mesh);return mesh;
  };
  ...
  if(!lod&&trees)for(let k=0;k<kinds.length;k++){
    ...
    batch(specimen.branchesMesh.geometry,specimen.branchesMesh.material,groups[k],`Garden trunks ${k}`,true,false);
    batch(specimen.leavesMesh.geometry,specimen.leavesMesh.material,groups[k],`Garden ${kinds[k]} crowns ${k}`,true,false);
  }
  const understory=[];
  ...
  // The retired tuft consumed five random values for each of its 32 blades.
  // Keep that exact advance so subsequent meadow flowers remain unchanged.
  for(let draw=0;draw<32*5;draw++)rand();
  const grassVariants=createGrassTuftVariants(),plantedGrass=grass.filter(placementClear);
  const grassDistribution=createGrassDistribution(plantedGrass,grassVariants,heightAt,{
    allowed:(x,z)=>!lowClear(x,z)&&placementClear({x,z})&&herbariumCommunitySoilAt(x,z)<=.30,
    reserved:herbariumArrivalReserved,lawnAt:herbariumLawnAt,openingAt:(x,z)=>plantingCommunityAt(x,z).opening,
  });
  const grassGroups=grassDistribution.groups;
  const gm=createGrassMaterial();
  grassVariants.forEach((geometry,variant)=>{
    const mesh=batch(geometry,gm,grassGroups[variant],'Grouped silver green ground cover',false);
    if(mesh){mesh.name+=` / variant ${variant+1}`;mesh.userData.grassVariant=variant;mesh.userData.grassSourceStride=grassVariants.length;mesh.userData.grassOriginalCount=grassDistribution.originalCounts[variant];}
