...
const softwareProjects=[{
  ...
  contribution:bi('Authored early specifications for the method, Web UI, and multi-round refinement. Added Codex and Claude runners, event-bus and session-state coverage, and refinement integration tests to establish a verifiable baseline for subsequent agent iterations.','负责早期方法、Web UI 与多轮 refinement 的项目规格，并补充 Codex / Claude runner、事件总线、会话状态和 refine 集成测试，为后续智能体迭代建立可验证的基线。'),
  links:[{label:bi('Code','代码'),url:'https://github.com/VILA-Lab/FigMirror'}],
}];
...
const details={
  autodesign:{shortTitle:bi('AutoDesign','AutoDesign'),category:bi('Agentic design','智能体设计'),
    ...
    approach:bi('A model-agnostic DesignHarness supports Codex, Claude Code, Kimi Code and other coding harnesses. The inner Designer–Critic loop refines native artifacts; outer Meta-Harness Optimization uses traces, evaluators and persistent optimization records to update reusable infrastructure, with acceptance gates preventing regressions.','模型无关的 DesignHarness 支持 Codex、Claude Code、Kimi Code 等 coding harness。内层 Designer–Critic 迭代原生产物，外层 Meta-Harness Optimization 基于轨迹、评测器与持久优化记录更新可复用系统，并通过接受门控约束回退。'),
    output:bi('Editable posters, slides, webpages and captioned videos. On the 100-paper PosterBench, public results reach 78.32: 7.45 points above Claude Design in the matched configuration, with gains of 5.01–19.56 points across seven matched configurations. These quantitative results concern posters; the other formats are demonstrated outputs.','可编辑海报、幻灯片、网页与带字幕视频。在 100 篇论文的 PosterBench 上，公开结果最高为 78.32 分，较同配置 Claude Design 高 7.45 分，7 组匹配配置增益为 5.01–19.56 分。这些量化结果针对海报；其他格式提供真实产物示例。'),
    media:[
  ...
  figmirror:{shortTitle:bi('FigMirror','FigMirror'),category:bi('Scientific figure agents','科研绘图智能体'),
    question:bi('How can an agent turn data and a visual reference into an editable, publication-ready figure?','智能体如何将数据与视觉参考转化为可编辑、可发表的科研图表？'),
    approach:bi('Orchestrator–Drawer–Reviewer roles coordinate a reference-driven plotting workflow. Grounded Measurement and multi-round refinement guide the conversion of user data into editable Matplotlib code.','通过 Orchestrator–Drawer–Reviewer 角色分工组织参考图驱动的绘图流程，结合 Grounded Measurement 与多轮 refinement，将用户数据转化为可编辑 Matplotlib 代码。'),
    output:bi('Editable Matplotlib source and camera-ready PDFs. The image shown here is an actual generated output from the official FigMirror showcase.','可编辑 Matplotlib 源代码与可直接用于论文的 PDF。这里展示的图片来自 FigMirror 官方 showcase 的真实生成产物。'),
    media:[{src:'/media/exhibitions/figmirror-generated.jpg',title:bi('A figure generated with FigMirror','FigMirror 生成的科研图表'),caption:bi('An actual generated figure from the official project showcase. The source repository presents its reference and output together.','官方项目 showcase 中的真实生成图表；来源仓库同时提供参考图与生成结果。'),kind:'output',source:'https://github.com/VILA-Lab/FigMirror/blob/d37edebbc7cb80acecfd949d3a3615f44c13929a/docs/assets/showcase/primary-generated.jpg',sourceLabel:bi('FigMirror · official showcase','FigMirror · 官方展示')}],
  },
