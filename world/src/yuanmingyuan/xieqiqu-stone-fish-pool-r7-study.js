...
import {createStoneFishCarvedWaveR7} from './xieqiqu-stone-fish-pool-r7-geometry.js';
import {stoneFishPoolR7StudyId} from './xieqiqu-stone-fish-pool-r7-views.js';
import {createXieqiquPoolWaterStudy} from './xieqiqu-pool-water-study.js';

export {prepareXieqiquStoneFishMaterialPixels};
export {stoneFishPoolR7StudyId,stoneFishPoolR7StudyViews} from './xieqiqu-stone-fish-pool-r7-views.js';
const check=(ok,message)=>{if(!ok)throw new Error('Stone fish pool R7: '+message);};
...
export function createXieqiquStoneFishPoolR7FromBase({baseOwner,signal}={}){
  ...
  try{
    ...
    for(const mount of baseOwner.context.mounts){
      ...
      let core=coreByBottom.get(key);if(!core){core=createStoneFishCarvedWaveR7({contactGeometry:baseOwner.seat.geometry,contactInfo:baseOwner.seat.diagnostics,bottomY,waterY,bodyMinimumY,worldScale:mount.placement.size});coreByBottom.set(key,core);geometryOwners.set(core.geometry,core);}
      for(const node of [...mount.plinth.children]){retired.push(node);mount.plinth.remove(node);}
      const carved=material('carved'),wet=material('wetFoot'),coreMesh=new THREE.Mesh(core.geometry,[carved,wet]);coreMesh.name=mount.group.name+'-carved-wave-body';coreMesh.userData={body:'closed-carved-wave-support-with-original-belly-contact',historicalCarvingVerified:false};bind(coreMesh,[carved,wet],mount.group);mount.plinth.add(coreMesh);supports.push({mount,core,coreMesh,crestMeshes:[]});
    }
    ...
    group.name=stoneFishPoolR7StudyId+'-study';group.userData={...group.userData,assetId:stoneFishPoolR7StudyId,visualAcceptance:false,historicalCarvingVerified:false};group.updateMatrixWorld(true);signal?.throwIfAborted();
    function validate(){
    ...
    const diagnostics={...baseOwner.diagnostics,assetId:stoneFishPoolR7StudyId,triangles,triangleCount:triangles,meshCount,bounds:(()=>{const b=new THREE.Box3().setFromObject(group);return {min:b.min.toArray(),max:b.max.toArray()};})(),sourceR1Assembly:baseOwner.diagnostics,materialStudy:'original fish marble-r1 plus unchanged R4 support/rim settings; R7 geometry only',contextMaterials:structuredClone(settings),supportStudy:{historicalCarvingVerified:false,construction:'unchanged real belly cap, 10 mm collar, R6 connector and submerged roots; rear-crossing lip alone gains arc-length lateral taper and rounded cross-section in one joined support per fish',uniqueWaveCoreGeometries:coreByBottom.size,sharedCrestGeometries:0,newMaterials:materials.size,newTextures:0,retiredSourceNodes:retired.length},waterStudy:waterOwner.diagnostics,resourceOwnership:{base:'owned exclusively; includes original six fish geometries and three marble textures',additionalSupportGeometry:geometryOwners.size,additionalSupportMaterials:materials.size,additionalTextures:0,allNewMaterialMaps:'borrowed from base texture owner',water:'separate owner; restored and disposed before support and base',retiredOriginalSupportResources:'owned by base, undrawn, released on disposal'},supportVariant:'r7',r7Validation:validate(),visualAcceptance:false,integrationAcceptance:false,archiveCompatible:false};
  ...
  }catch(error){try{dispose();}catch(cleanup){throw new AggregateError([error,cleanup],'Stone fish pool R7 construction and cleanup failed',{cause:error});}throw error;}
}

export function createXieqiquStoneFishPoolR7Study({pixels,signal}={}){
  signal?.throwIfAborted();const baseOwner=createXieqiquStoneFishPoolStudy({pixels,signal});return createXieqiquStoneFishPoolR7FromBase({baseOwner,signal});
}
