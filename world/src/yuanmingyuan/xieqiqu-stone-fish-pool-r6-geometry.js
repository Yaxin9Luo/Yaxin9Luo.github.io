...
const check=(ok,message)=>{if(!ok)throw new Error('Stone fish wave R6: '+message);};
 ...
 * R6 changes only authored material below the exact cap and 10 mm collar.
 * It remains a modern artistic inference, never a measured historical carving. */
export function createStoneFishCarvedWaveR6({contactGeometry,contactInfo,bottomY,waterY,bodyMinimumY,worldScale=1,spacing=.008}={}){
  ...
  const geometry=new THREE.BufferGeometry();geometry.name='stone-fish-r6-tangent-continuous-shoulder';geometry.setAttribute('position',new THREE.Float32BufferAttribute(positions,3));geometry.setAttribute('uv',new THREE.Float32BufferAttribute(uv,2));geometry.setIndex([...dry,...wet]);geometry.addGroup(0,dry.length,0);geometry.addGroup(dry.length,wet.length,1);geometry.computeVertexNormals();
  ...
  geometry.userData={body:'single-three-folded-wave-support',supportVariant:'r6',historicalCarvingVerified:false,originalContactCapRetained:true,roofVertices:n,roofTriangles:roofCount/3,bottomY,waterY,worldScale,waveCount:waves.length,maximumWaveY,broadenedSubmergedFoot:false,spacing,diagonalRepairs,fieldGrid:[nx,ny,nz],distanceGridBytes:profiles.reduce((sum,p)=>sum+p.bytes,0)+values.byteLength,authoredRevision:'rounded-cap-corners-below-fixed-strip-r6',transitionContinuity:'zero radial slope at original vertical collar and vertical field bearing',transitionRows,cornerRoundness:{maximumWeight:Math.max(...cornerWeights),affectedBoundaryVertices:cornerWeights.filter(w=>w>0).length,maximumTravelGain:.16},capCollar:{start:collarStart,count:collar.length,depth:.010,sourceBoundary:upper.slice()},normalSource:'area-weighted shared Float32 surface normals; unchanged cap normals'};
  let disposed=false;
  const validate=()=>{
