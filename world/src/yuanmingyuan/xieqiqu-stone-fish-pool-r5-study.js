...
import {createStoneFishCarvedWaveR5} from './xieqiqu-stone-fish-pool-r5-geometry.js';
import {stoneFishPoolR5StudyId} from './xieqiqu-stone-fish-pool-r5-views.js';
import {createXieqiquPoolWaterStudy} from './xieqiqu-pool-water-study.js';

export {prepareXieqiquStoneFishMaterialPixels};
export {stoneFishPoolR5StudyId,stoneFishPoolR5StudyViews} from './xieqiqu-stone-fish-pool-r5-views.js';
const check=(ok,message)=>{if(!ok)throw new Error('Stone fish pool R5: '+message);};
...
export function createXieqiquStoneFishPoolR5FromBase({baseOwner,signal}={}){
  ...
  try{
    ...
    for(const mount of baseOwner.context.mounts){
      ...
      let core=coreByBottom.get(key);if(!core){core=createStoneFishCarvedWaveR5({contactGeometry:baseOwner.seat.geometry,contactInfo:baseOwner.seat.diagnostics,bottomY,waterY,bodyMinimumY,worldScale:mount.placement.size});coreByBottom.set(key,core);geometryOwners.set(core.geometry,core);}
    ...
    group.name=stoneFishPoolR5StudyId+'-study';group.userData={...group.userData,assetId:stoneFishPoolR5StudyId,visualAcceptance:false,historicalCarvingVerified:false};group.updateMatrixWorld(true);signal?.throwIfAborted();
    function validate(){
  ...
  }catch(error){try{dispose();}catch(cleanup){throw new AggregateError([error,cleanup],'Stone fish pool R5 construction and cleanup failed',{cause:error});}throw error;}
}

export function createXieqiquStoneFishPoolR5Study({pixels,signal}={}){
  signal?.throwIfAborted();const baseOwner=createXieqiquStoneFishPoolStudy({pixels,signal});return createXieqiquStoneFishPoolR5FromBase({baseOwner,signal});
}
