...
const check=(ok,message)=>{if(!ok)throw new Error('Stone fish wave R4: '+message);};
...
export function createStoneFishCarvedWaveR4({contactGeometry,contactInfo,bottomY,waterY,bodyMinimumY,worldScale=1,spacing=.008}={}){
  ...
  const waveDistance=(i,x,y,z)=>{
    ...
    const d=profiles[i].sample(u,y),flowRelief=.0055*Math.sin((d+.025)*TAU/.090)*(1-smooth(d,-.015,.010))*smooth(y,waterY-.07,waterY+.035),w=Math.abs(lateral-bulge)-width-flowRelief;return Math.hypot(Math.max(d,0),Math.max(w,0))+Math.min(Math.max(d,w),0)-.009;
  };
  const coreDistance=(x,y,z)=>{
    const t=smooth(safeY-y,0,safeY-waterY),below=smooth(waterY-y,0,waterY-bottomY),rx=lerp(headX,.078,t)+.012*below,rz=lerp(headZ,.230,t)+.028*below,cx=centre.x-.025*Math.sin(Math.PI*t),cz=centre.y+.027*Math.sin(Math.PI*t);
  ...
  for(let row=1;row<=11;row++){
    const t=row/12,ring=upper.map(id=>{const p=point(id),end=radialTarget(Math.atan2(p.z-centre.y,p.x-centre.x)),x=lerp(p.x,end[0],t),z=lerp(p.z,end[1],t),capY=capHeightAt(x,z);check(Number.isFinite(capY),'cap transition left original footprint');return add(x,Math.min(capY-.010,lerp(p.y-.010,Math.min(capY-.010,end[2]),smooth(t,0,1))),z);});
  ...
  geometry.userData={body:'single-three-folded-wave-support',supportVariant:'r4',historicalCarvingVerified:false,originalContactCapRetained:true,roofVertices:n,roofTriangles:roofCount/3,bottomY,waterY,worldScale,waveCount:waves.length,maximumWaveY,broadenedSubmergedFoot:false,spacing,diagonalRepairs,fieldGrid:[nx,ny,nz],distanceGridBytes:profiles.reduce((sum,p)=>sum+p.bytes,0)+values.byteLength,normalSource:'area-weighted shared Float32 surface normals; unchanged cap normals'};
  let disposed=false;
  const validate=()=>{
    ...
    return {unchangedRoofVertices:n,unchangedRoofTriangles:roofCount/3,maximumContactDrift:0,checkedNeckVertices:upperVertices,maximumNeckAboveContact:maxAbove,minimumWaveClearance,originalFloorY:bottomY,actualFloorY:geometry.boundingBox.min.y,joinedWaveRoots:waves.length};
  };
