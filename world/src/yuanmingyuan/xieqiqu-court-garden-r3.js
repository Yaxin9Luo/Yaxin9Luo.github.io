...
import {westernGardenPlantingSourceReview} from './western-garden-planting.js';
...
function readPlant(part,signal,species){
  ...
  let triangles=0,trunks=0;
  part.traverse(node=>{
    if(!node.isMesh)return;signal?.throwIfAborted();
    if(node.isInstancedMesh&&species!=='juniper'||node.isSkinnedMesh||node.isBatchedMesh||node.morphTargetInfluences||!visibleWithin(node,part)||node.material?.visible===false)throw new Error('Court garden requires complete visible static understory source meshes');
    ...
    const matrix=inverse.clone().multiply(node.matrixWorld),base=g.boundingBox?.clone()??new THREE.Box3().setFromBufferAttribute(p);
    let localBox=base;
    if(node.isInstancedMesh){
      if(!Number.isInteger(node.count)||node.count<=0||node.instanceMatrix?.count<node.count)throw new Error('Incomplete original juniper instance attribute');
      localBox=node.boundingBox?.clone()??new THREE.Box3();
      if(!node.boundingBox)for(let i=0;i<node.count;i++){
        if(i%2048===0)signal?.throwIfAborted();const transform=new THREE.Matrix4();node.getMatrixAt(i,transform);
        if(!transform.elements.every(Number.isFinite)||transform.determinant()<=0)throw new Error('Invalid original juniper instance transform');
        localBox.union(base.clone().applyMatrix4(transform));
      }
    }
    const box=localBox.applyMatrix4(matrix);
    bounds.union(box);triangles+=count/3*(node.isInstancedMesh?node.count:1);records.push({node,matrix});
    if(species==='juniper'){
      if(node.name!=='juniper-visible-trunk')return;
      if(node.isInstancedMesh)throw new Error('Juniper collision/root body must be original static wood');trunks++;
    }
    if(node.isInstancedMesh||box.min.y>.005)return;
    for(let i=0;i<p.count;i++){
      if(i%8192===0)signal?.throwIfAborted();const q=V().fromBufferAttribute(p,i).applyMatrix4(matrix);
      if(q.y>(species==='juniper'?0:.005))continue;const key=[q.x,q.y,q.z].map(v=>Math.round(v*1e6)).join(',');
      if(!seenRoots.has(key)){seenRoots.add(key);roots.push(q);}
    }
  });
  if(species==='juniper'&&trunks!==1)throw new Error('Exactly one original juniper-visible-trunk is required');
 ...
 */
export function createXieqiquCourtGardenR1ArchitectureAudit(owner,{signal,localBounds}={}){
  ...
  const requested=localBounds??new THREE.Box3(V(-32,.023,-44),V(32,3,-16.1));
  if(requested.isEmpty()||![...requested.min.toArray(),...requested.max.toArray()].every(Number.isFinite))throw new Error('Invalid court architecture audit bounds');
  const region=requested.clone().applyMatrix4(root.matrixWorld),instances=new THREE.Matrix4();
  ...
  return {get disposed(){return support.disposed;},dispose:()=>support.dispose(),get diagnostics(){return {...support.diagnostics,boundedNorthCourt:true,requestedLocalBounds:{min:requested.min.toArray(),max:requested.max.toArray()}};},
 ...
 */
export async function prepareXieqiquCourtGardenR1({owner,sources,layout=createXieqiquCourtGardenR1Layout(),architecture,buildSupport=true,signal,onProgress=()=>{},yieldControl=yieldTask,createSoilMaterial,broadleafContract=courtBroadleafContract}={}){
  signal?.throwIfAborted();
  const contract=assertCourtBroadleafContract(broadleafContract),sourceReview=contract.review,rootMinY=contract.profile.rootMinY;
  const root=owner?.group;
  if(!root?.isObject3D||owner.disposed||!root.children.length||owner.collisionGroup&&owner.collisionGroup!==root||typeof buildSupport!=='boolean'||createSoilMaterial!==undefined&&typeof createSoilMaterial!=='function'||architecture&&(typeof architecture.intersectsGuideVolume!=='function'||architecture.disposed)||active.has(root))throw new Error('A complete unused placed Xieqiqu owner and current source support are required');
  ...
  const plan=layout,group=new THREE.Group(),stage=new THREE.Group(),geometries=new Set(),materials=new Set(),textures=new Set(),instances=[],borrowedInstances=[],listeners=new Map(),sourceOwners=new Set();
  const diagnostics={id:plan.id,evidence:plan.evidence,nativeReviewed:false,historicallySurveyed:false,sourceGeometryChanged:false,
    ...
    architectureChecks:0,actualSourceTriangles:0,drawnPlantTriangles:0,ownedSurfaceTriangles:0,mineralPatternCount:0,mineralPatternArea:0,sourceInvalidated:false,materialRestored:false,errors:[]};
  ...
  let disposed=false,displayMaterial,support=null,supportFrame=null,reader,ownedAudit=null,soilLease=null,soilLeaseReleased=false;
  function releaseSoilLease(){if(soilLease&&!soilLeaseReleased){soilLeaseReleased=true;if(typeof soilLease.dispose==='function'&&!soilLease.disposed)soilLease.dispose();}}
  ...
  function dispose(){
    ...
    for(const {copy,emptyMatrix}of borrowedInstances)run(()=>{
      // r185 removes instance GPU buffers on dispose. Detach borrowed original
      // attributes before deleting this view so the source/other views survive.
  ...
  function guard(){
    signal?.throwIfAborted();
    if(disposed||owner.disposed||soilLease?.disposed||!root.children.length||[...sourceOwners].some(value=>value.disposed)||reader&&!readIdentity())throw new Error('Court garden source/floor was invalidated');
  }
  ...
  function juniperView(node,bedId){
    let copy;
    if(node.isInstancedMesh){
      ...
      borrowedInstances.push({copy,emptyMatrix});
    ...
    else throw new Error('Unsupported original juniper node '+node.type);
    ...
    const trunk=node.isMesh&&!node.isInstancedMesh&&node.name==='juniper-visible-trunk';
    copy.userData={...copy.userData,navigation:trunk,courtGardenTrunk:trunk,borrowedOriginalGeometry:!!node.isMesh,sourceSpecies:'juniper',sourceBedId:bedId};
    for(const child of node.children)copy.add(juniperView(child,bedId));
    return copy;
  }
  ...
  try{
    ...
    const required=[...new Set(plan.placements.map(p=>p.species))];
    if(required.some(id=>!['sedge','flower-shrub','juniper','low-broadleaf'].includes(id)))throw new Error('Unsupported court garden species');
    for(const species of required){
      guard();const source=sources?.[species];
      if(!source?.part?.isObject3D||source.part.userData.id!==species||!source.owner||source.owner.disposed||typeof source.review!=='string'||source.review!==(species==='low-broadleaf'?sourceReview:westernGardenPlantingSourceReview(species)))throw new Error('The full reviewed understory source is required: '+species);
      sourceOwners.add(source.owner);const profile=readPlant(source.part,signal,species);if(species==='low-broadleaf')contract.assertGeometry(source.part,profile);profiles.set(species,profile);diagnostics.actualSourceTriangles+=profile.triangles;
    ...
    const auditBounds=new THREE.Box3();
    for(const bed of plan.beds){const bounds=ringBox(bed.apron);bounds.min.y=.023;bounds.max.y=plan.levels.borderTop+.01;auditBounds.union(bounds);}
    for(const p of plan.placements){
      if(![p.x,p.z,p.scale,p.yaw].every(Number.isFinite)||p.scale<=0||!plan.beds.some(b=>b.id===p.bedId))throw new Error('Invalid complete court placement '+p.id);
      const matrix=new THREE.Matrix4().compose(V(p.x,plan.levels.soilTop-plan.levels.rootBurial,p.z),new THREE.Quaternion().setFromAxisAngle(V(0,1,0),p.yaw),V(p.scale,p.scale,p.scale));
      const bounds=profiles.get(p.species).bounds.clone().applyMatrix4(matrix);bounds.min.y=Math.max(.023,bounds.min.y);auditBounds.union(bounds);
    }
    auditBounds.min.x-=.05;auditBounds.min.z-=.05;auditBounds.max.x+=.05;auditBounds.max.z+=.05;auditBounds.max.y+=.05;
    diagnostics.actualAuditLocalBounds={min:auditBounds.min.toArray(),max:auditBounds.max.toArray()};
    if(!architecture){ownedAudit=createXieqiquCourtGardenR1ArchitectureAudit(owner,{signal,localBounds:auditBounds});architecture=ownedAudit;}
    ...
    let soilMaterial;
    if(createSoilMaterial){
      soilLease=await createSoilMaterial({signal,layout:plan});
      // An uncooperative asynchronous factory may settle after cancellation.
      // Its newly returned lease still belongs to this failed transaction.
      if(disposed||signal?.aborted){releaseSoilLease();guard();}
      guard();
      if(!soilLease?.material?.isMeshStandardMaterial||typeof soilLease.dispose!=='function'||soilLease.disposed!==false)throw new Error('Soil factory must return a live exclusive material lease');
      soilMaterial=soilLease.material;diagnostics.soilMaterialLease=soilLease.diagnostics??null;
    }else soilMaterial=defaultSoilMaterial();
    // A shallow mineral colour/grain study on the same soil height. It borrows
    // the original stone texture through one private display material; no
    // extra slab, terrain hole or navigation body is introduced.
    const hasMineralPatterns=plan.beds.some(bed=>bed.mineralPatches?.length);
    const mineralMaterial=hasMineralPatterns?material(originalMaterial,'restrained-mineral-surface-pattern',0x777a69):null;
    if(mineralMaterial){mineralMaterial.roughness=.98;mineralMaterial.polygonOffset=true;mineralMaterial.polygonOffsetFactor=-1;mineralMaterial.polygonOffsetUnits=-1;mineralMaterial.depthWrite=false;}
    const beds=new Map();
    for(const bed of plan.beds){
      ...
      for(const pattern of bed.mineralPatches??[]){
        if(pattern.kind!=='surface-mineral-pattern'||!Array.isArray(pattern.ring)||pattern.ring.length<3||
          pattern.ring.some(p=>!Array.isArray(p)||p.length!==2||!p.every(Number.isFinite)||!pointInPolygon(p,bed.inner))||area(pattern.ring)<=0)
          throw new Error('Mineral pattern must remain inside the actual continuous bed soil: '+pattern.id);
        const geometry=new THREE.ShapeGeometry(new THREE.Shape(pattern.ring.map(([x,z])=>new THREE.Vector2(x,-z))),20);
        geometry.rotateX(-Math.PI/2);geometry.translate(0,plan.levels.soilTop,0);
        const view=mesh(pattern.id,geometry,mineralMaterial);
        view.castShadow=false;view.renderOrder=1;
        view.userData={courtGardenSurface:false,navigation:false,courtMineralPattern:true,bedId:bed.id,patternId:pattern.id,
          evidence:'contemporary-surface-pattern-not-historic-paving',support:'existing-complete-bed-soil'};
        diagnostics.mineralPatternCount++;diagnostics.mineralPatternArea+=area(pattern.ring);
      }
    ...
    for(const species of required){
      ...
      for(const p of placements){
        ...
        for(const rootPoint of profile.roots){
          ...
          if((bed.mineralPatches??[]).some(pattern=>pointInPolygon([point.x,point.z],pattern.ring)))throw new Error('Original plant root enters reserved surface mineral pattern: '+p.id);
          ...
          if(gap>.004||gap<(species==='juniper'?-.21*p.scale-plan.levels.rootBurial:species==='low-broadleaf'?rootMinY*p.scale-plan.levels.rootBurial-.000002:-.035))throw new Error('Original root does not meet actual soil surface: '+p.id);
        }
        if(species==='juniper'){
          const placement=new THREE.Group();placement.name=plan.id+'-'+p.id;
          placement.userData={placementId:p.id,species:'juniper',sourceBedId:p.bedId};
          placement.matrixAutoUpdate=false;placement.matrix.copy(matrix);
          placement.add(juniperView(sources.juniper.part,p.bedId));group.add(placement);
        }else matrices.get(p.bedId).push(matrix);
      }
      for(const bed of plan.beds){
    ...
    if(buildSupport){support=createArchitectureSurface(group,{include:node=>node.userData.courtGardenSurface===true||node.userData.courtGardenTrunk===true});supportFrame=root.matrixWorld.clone();}
    ...
    const binding={group,plan,diagnostics,dispose,get support(){return disposed?null:support;},get disposed(){return disposed;},
      createSupport(){
        ...
        support=createArchitectureSurface(group,{include:node=>node.userData.courtGardenSurface===true||node.userData.courtGardenTrunk===true});supportFrame=root.matrixWorld.clone();return support;
      },
      assertCurrent(){
