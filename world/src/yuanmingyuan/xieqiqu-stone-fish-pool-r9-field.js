...
const check=(ok,message)=>{if(!ok)throw new Error('Stone fish wave R9 field: '+message);};
...
// A shallow broken crest with one short returned free edge, rather than the
// old low semicircular return. The rising back and underwater foot stay broad.
function shallowRearProfile(bottomY,waterY,scale){
  const points=[[-.24*scale,bottomY-.04]],v=(x,y)=>new THREE.Vector3(x*scale,waterY+y*scale,0);
  const curve=(a,b,end)=>{const last=points.at(-1),c=new THREE.CubicBezierCurve3(new THREE.Vector3(last[0],last[1],0),a,b,end);for(let i=1;i<=24;i++){const p=c.getPoint(i/24);points.push([p.x,p.y]);}};
  curve(new THREE.Vector3(-.25*scale,bottomY*.55,0),v(-.405,-.075),v(-.38,-.002));
  curve(v(-.34,.04),v(-.22,.19),v(-.14,.25));
  curve(v(-.075,.319),v(.10,.329),v(.18,.280));
  curve(v(.230,.260),v(.275,.235),v(.254,.200));
  curve(v(.242,.178),v(.215,.169),v(.191,.178));
  curve(v(.139,.197),v(.082,.156),v(.058,.076));
  curve(v(.06,-.03),v(.14,-.09),new THREE.Vector3(.25*scale,bottomY-.04,0));
  return points;
}
...
// Progress follows the authored rolled lip rather than world height. This
// scalar grid does not replace or simplify the retained 2.5 mm profile field.
const rearLipStations=[[-.14,.25],[-.04,.285],[.08,.291],[.18,.268],[.239,.202],[.224,.098]];
function createRearLipProgress(scale,level){
  const curve=new THREE.CatmullRomCurve3(rearLipStations.map(([u,v])=>new THREE.Vector3(u*scale,level+v*scale,0)),false,'centripetal'),points=curve.getSpacedPoints(96),x0=-.19*scale,x1=.32*scale,y0=level+.035*scale,y1=level+.355*scale,step=.0025,nx=Math.ceil((x1-x0)/step),ny=Math.ceil((y1-y0)/step),stride=nx+1,progress=new Float32Array((nx+1)*(ny+1)),distance=new Float32Array(progress.length);
  for(let j=0;j<=ny;j++)for(let i=0;i<=nx;i++){const x=lerp(x0,x1,i/nx),y=lerp(y0,y1,j/ny);let best=Infinity,t=0;for(let k=0;k<points.length-1;k++){const a=points[k],b=points[k+1],vx=b.x-a.x,vy=b.y-a.y,u=THREE.MathUtils.clamp(((x-a.x)*vx+(y-a.y)*vy)/(vx*vx+vy*vy),0,1),d=(x-a.x-vx*u)**2+(y-a.y-vy*u)**2;if(d<best){best=d;t=(k+u)/(points.length-1);}}const index=i+stride*j;progress[index]=t;distance[index]=Math.sqrt(best);}
  const sample=(values,x,y,outside)=>{const a=(x-x0)/(x1-x0)*nx,b=(y-y0)/(y1-y0)*ny;if(a<0||b<0||a>=nx||b>=ny)return outside;const i=Math.floor(a),j=Math.floor(b),u=a-i,v=b-j,k=i+stride*j;return lerp(lerp(values[k],values[k+1],u),lerp(values[k+stride],values[k+stride+1],u),v);};
  return {sampleProgress:(x,y)=>sample(progress,x,y,0),sampleDistance:(x,y)=>sample(distance,x,y,Infinity),bytes:progress.byteLength+distance.byteLength,stations:rearLipStations.map(([u,v])=>[u*scale,level+v*scale])};
}
 ...
 * This temporary field carries the original cap solely for assembly. The R9
 * public owner applies the separately reviewed R8 buried rounded contact.
 * It remains a modern artistic inference, never a measured historical carving. */
export function createStoneFishCarvedWaveR9Field({contactGeometry,contactInfo,bottomY,waterY,bodyMinimumY,worldScale=1,spacing=.008}={}){
  ...
  const shallowProfile=profileDistanceGrid(shallowRearProfile(bottomY,waterY-specs[1].drop,specs[1].scale)),profiles=specs.map(s=>profileDistanceGrid(waveProfile(bottomY,waterY-s.drop,s.scale))),rearLip=createRearLipProgress(specs[1].scale,waterY-specs[1].drop);
  ...
  const refinedRearLipDistance=(x,y,z,original,otherDistance)=>{
    // The full original field is exact below the first two 8 mm air cells,
    // throughout the original fusion band, and near the fixed cap boundary.
    const air=smooth(y,waterY+2*spacing,waterY+.060)*(1-smooth(y,waterY+.260,waterY+.290)),dominance=smooth(otherDistance-original,.030,.060);if(!air||!dominance)return original;
    const s=specs[1],c=Math.cos(s.angle),a=Math.sin(s.angle),dx=x-centre.x-s.x,dz=z-centre.y-s.z,u=dz*c+dx*a,lateral=dx*c-dz*a,bulge=.037*Math.sin((y-waterY)*7+.6)+.020*Math.sin(u*6),width=s.width*(.77+.23*smooth(y,waterY-.12,waterY+.22))+.009*Math.sin(u*11+(y-waterY)*8),q=THREE.MathUtils.clamp((lateral-bulge)/width,-1.1,1.1),edge=smooth(Math.abs(q+.10),.20,1.10),gate=smooth(y,waterY+.050,waterY+.150),taper=1-.10*edge,blend=1+gate*(taper-1),crown=(.022*(1-edge)-.008*edge+.004*q)*s.scale,bow=(.032*(1-q*q)+.029*q+.006*Math.sin(Math.PI*q))*s.scale,rise=.022*s.scale*smooth(y-waterY,.070,.290)*smooth(-u,0,.300*s.scale),profileY=(y-gate*((1-taper)*(waterY+.170)+crown))/blend,profileU=(u-gate*((1-taper)*(.135*s.scale)+bow+rise))/blend,progress=rearLip.sampleProgress(profileU,profileY),local=(1-smooth(rearLip.sampleDistance(profileU,profileY),.100*s.scale,.145*s.scale))*smooth(progress,.02,.22),weight=air*dominance*local;if(!weight)return original;
    const d=profiles[1].sample(profileU,profileY),flowRelief=.0055*Math.sin((d+.025)*TAU/.090)*(1-smooth(d,-.015,.010))*smooth(y,waterY-.07,waterY+.035),widthFactor=1-.10*smooth(progress,.08,.42)-.25*smooth(progress,.42,1),w=Math.abs(lateral-bulge)-width*(1-.07*gate)*widthFactor-flowRelief,k=.070*s.scale,rounded=-smoothMin(-d,-w,k)-.009;return lerp(original,rounded,weight);
  };
  const shallowRearDistance=(x,y,z)=>{
    const s=specs[1],c=Math.cos(s.angle),a=Math.sin(s.angle),dx=x-centre.x-s.x,dz=z-centre.y-s.z,u=dz*c+dx*a,lateral=dx*c-dz*a,bulge=.037*Math.sin((y-waterY)*7+.6)+.020*Math.sin(u*6),width=s.width*(.77+.23*smooth(y,waterY-.12,waterY+.22))+.009*Math.sin(u*11+(y-waterY)*8),q=THREE.MathUtils.clamp((lateral-bulge)/width,-1.1,1.1),edge=smooth(Math.abs(q+.10),.20,1.10),gate=smooth(y,waterY+.050,waterY+.150),taper=1-.10*edge,blend=1+gate*(taper-1),crown=(.022*(1-edge)-.008*edge+.004*q)*s.scale,bow=(.032*(1-q*q)+.029*q+.006*Math.sin(Math.PI*q))*s.scale,rise=.022*s.scale*smooth(y-waterY,.070,.290)*smooth(-u,0,.300*s.scale);
    const asym=smooth(y,waterY+.035,waterY+.100)*smooth(u,-.070*s.scale,.160*s.scale),profileY=(y-gate*((1-taper)*(waterY+.170)+crown))/blend+ .016*q*asym,profileU=(u-gate*((1-taper)*(.135*s.scale)+bow+rise))/blend-.045*q*asym,end=smooth(profileU,.030*s.scale,.240*s.scale),d=shallowProfile.sample(profileU,profileY),roundGate=smooth(y,waterY+.035,waterY+.145),widthFactor=lerp(1,lerp(.96,.60,end),roundGate),flowRelief=.0055*Math.sin((d+.025)*TAU/.090)*(1-smooth(d,-.015,.010))*smooth(y,waterY-.07,waterY+.035),w=Math.abs(lateral-bulge)-width*(1-.07*gate)*widthFactor-flowRelief*(1-roundGate),k=.065*s.scale;return lerp(Math.hypot(Math.max(d,0),Math.max(w,0))+Math.min(Math.max(d,w),0),-smoothMin(-d,-w,k),roundGate)-.009;
  };
  const field=(x,y,z)=>{const core=coreDistance(x,y,z),forward=waveDistance(0,x,y,z),rear=waveDistance(1,x,y,z),returning=waveDistance(2,x,y,z),other=Math.min(core,forward,returning),shaped=refinedRearLipDistance(x,y,z,rear,other),weight=smooth(y,waterY+2*spacing,waterY+.032)*(1-smooth(y,waterY+.235,waterY+.255)*(1-smooth(z,.59,.69))),newRear=weight?lerp(shaped,shallowRearDistance(x,y,z),weight):shaped;let d=smoothMin(core,forward,.030);d=smoothMin(d,newRear,.030);return smoothMin(d,returning,.030);};
  ...
  // Round only the cut-outline corners below the exact strip. Arc-length
  // neighbours suppress tiny scale-edge changes; each radial path remains
  // monotone, with vertical tangents at the strip and the unchanged bearing.
  const cornerWeights=upper.map((id,j)=>{const p=point(id),neighbour=step=>{let at=j,distance=0,previous=p;for(let k=0;k<upper.length&&distance<.012;k++){at=(at+step+upper.length)%upper.length;const q=point(upper[at]);distance+=Math.hypot(q.x-previous.x,q.z-previous.z);previous=q;}return previous;},a=neighbour(-1),b=neighbour(1),u=new THREE.Vector2(p.x-a.x,p.z-a.z).normalize(),v=new THREE.Vector2(b.x-p.x,b.z-p.z).normalize();return smooth(1-u.dot(v),.12,.65);}),transitionRows=24;
  for(let row=1;row<transitionRows;row++){
    const t=row/transitionRows,ring=upper.map((id,j)=>{const p=point(id),end=radialTarget(Math.atan2(p.z-centre.y,p.x-centre.x)),travel=smooth(t,0,1)+2.56*cornerWeights[j]*t*t*(1-t)*(1-t),x=lerp(p.x,end[0],travel),z=lerp(p.z,end[1],travel),capY=capHeightAt(x,z);check(Number.isFinite(capY),'cap transition left original footprint');return add(x,Math.min(capY-.010,lerp(p.y-.010,Math.min(capY-.010,end[2]),t)),z);});
  ...
  const geometry=new THREE.BufferGeometry();geometry.name='stone-fish-r9-resampled-asymmetric-shallow-crest-field';geometry.setAttribute('position',new THREE.Float32BufferAttribute(positions,3));geometry.setAttribute('uv',new THREE.Float32BufferAttribute(uv,2));geometry.setIndex([...dry,...wet]);geometry.addGroup(0,dry.length,0);geometry.addGroup(dry.length,wet.length,1);geometry.computeVertexNormals();
  ...
  geometry.userData={body:'single-three-folded-wave-support',supportVariant:'r9-field',historicalCarvingVerified:false,originalContactCapRetained:true,roofVertices:n,roofTriangles:roofCount/3,bottomY,waterY,worldScale,waveCount:waves.length,maximumWaveY,broadenedSubmergedFoot:false,spacing,diagonalRepairs,fieldGrid:[nx,ny,nz],distanceGridBytes:profiles.reduce((sum,p)=>sum+p.bytes,0)+shallowProfile.bytes+values.byteLength+rearLip.bytes,authoredRevision:'asymmetric-shallow-rear-crest-field-r9-seventh',r9RearCrest:{authoredFreeEdgeProfileY:[waterY-.012+.178*.73,waterY-.012+.200*.73],lateralPhase:.045,verticalPhase:.016,widthFactors:[.96,.60],widthRoundingHeight:[.035,.145],fusion:'smooth union across actual new profile; retired old-field dominance freeze',upperShoulderProtection:{height:[.235,.255],longitudinal:[.59,.69]},minimumProtectedAir:2*spacing,profileGridSpacing:.0025},rearLip:{wave:specs[1].id,widthFactors:[1,.90,.65],roundingK:.070*specs[1].scale,minimumProtectedAir:2*spacing,fullAirGate:.060,upperFade:[.260,.290],protectedFusionGap:.030,fullDominanceGap:.060,stations:rearLip.stations,scalarGridBytes:rearLip.bytes},transitionContinuity:'zero radial slope at original vertical collar and vertical field bearing',transitionRows,cornerRoundness:{maximumWeight:Math.max(...cornerWeights),affectedBoundaryVertices:cornerWeights.filter(w=>w>0).length,maximumTravelGain:.16},capCollar:{start:collarStart,count:collar.length,depth:.010,sourceBoundary:upper.slice()},normalSource:'area-weighted shared Float32 surface normals; unchanged cap normals'};
  let disposed=false;
  const validate=()=>{
