...
import {createStoneFishCarvedWaveR8} from './xieqiqu-stone-fish-pool-r8-geometry.js';
import {stoneFishPoolR8StudyId} from './xieqiqu-stone-fish-pool-r8-views.js';
import {createXieqiquPoolWaterStudy} from './xieqiqu-pool-water-study.js';

export {prepareXieqiquStoneFishMaterialPixels};
export {stoneFishPoolR8StudyId,stoneFishPoolR8StudyViews} from './xieqiqu-stone-fish-pool-r8-views.js';
const check=(ok,message)=>{if(!ok)throw new Error('Stone fish pool R8: '+message);};
...
export function createXieqiquStoneFishPoolR8FromBase({baseOwner,signal}={}){
  ...
  try{
    ...
    for(const mount of baseOwner.context.mounts){
      ...
      let core=coreByBottom.get(key);if(!core){core=createStoneFishCarvedWaveR8({sourceGeometry:body,contactGeometry:baseOwner.seat.geometry,contactInfo:baseOwner.seat.diagnostics,bottomY,waterY,bodyMinimumY,worldScale:mount.placement.size});coreByBottom.set(key,core);geometryOwners.set(core.geometry,core);}
    ...
    group.name=stoneFishPoolR8StudyId+'-study';group.userData={...group.userData,assetId:stoneFishPoolR8StudyId,visualAcceptance:false,historicalCarvingVerified:false};group.updateMatrixWorld(true);signal?.throwIfAborted();
    function validate(){
    ...
    const diagnostics={...baseOwner.diagnostics,assetId:stoneFishPoolR8StudyId,triangles,triangleCount:triangles,meshCount,bounds:(()=>{const b=new THREE.Box3().setFromObject(group);return {min:b.min.toArray(),max:b.max.toArray()};})(),sourceR1Assembly:baseOwner.diagnostics,materialStudy:'original fish marble-r1 plus unchanged R4 support/rim settings; R8 geometry only',contextMaterials:structuredClone(settings),supportStudy:{historicalCarvingVerified:false,construction:'smaller rounded neck and smooth hidden closure inside the complete original fish; old cap and visible 10 mm skirt deliberately replaced; R7 wave positions and original submerged roots retained',uniqueWaveCoreGeometries:coreByBottom.size,sharedCrestGeometries:0,newMaterials:materials.size,newTextures:0,retiredSourceNodes:retired.length},waterStudy:waterOwner.diagnostics,resourceOwnership:{base:'owned exclusively; includes original six fish geometries and three marble textures',additionalSupportGeometry:geometryOwners.size,additionalSupportMaterials:materials.size,additionalTextures:0,allNewMaterialMaps:'borrowed from base texture owner',water:'separate owner; restored and disposed before support and base',retiredOriginalSupportResources:'owned by base, undrawn, released on disposal'},seat:{kind:'R8 smooth hidden closure and one continuous neck',historicalFormVerified:false,retiredSourceCap:'original cap remains owned by base but is not drawn',contacts:supports.map(s=>({index:s.mount.placement.index,...s.core.validate()}))},bindings:baseOwner.diagnostics.bindings.map(binding=>({...binding,seatEmbedding:supports.find(s=>s.mount.placement.index===binding.index).core.validate().worldEmbedding,seatEmbeddingMethod:'full original body BVH vertical containment of R8 hidden closure'})),supportVariant:'r8',r8Validation:validate(),visualAcceptance:false,integrationAcceptance:false,archiveCompatible:false};
  ...
  }catch(error){try{dispose();}catch(cleanup){throw new AggregateError([error,cleanup],'Stone fish pool R8 construction and cleanup failed',{cause:error});}throw error;}
}

export function createXieqiquStoneFishPoolR8Study({pixels,signal}={}){
  signal?.throwIfAborted();const baseOwner=createXieqiquStoneFishPoolStudy({pixels,signal});return createXieqiquStoneFishPoolR8FromBase({baseOwner,signal});
}
