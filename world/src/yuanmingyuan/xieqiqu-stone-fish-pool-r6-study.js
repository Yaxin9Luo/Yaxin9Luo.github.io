...
import {createStoneFishCarvedWaveR6} from './xieqiqu-stone-fish-pool-r6-geometry.js';
import {stoneFishPoolR6StudyId} from './xieqiqu-stone-fish-pool-r6-views.js';
import {createXieqiquPoolWaterStudy} from './xieqiqu-pool-water-study.js';

export {prepareXieqiquStoneFishMaterialPixels};
export {stoneFishPoolR6StudyId,stoneFishPoolR6StudyViews} from './xieqiqu-stone-fish-pool-r6-views.js';
const check=(ok,message)=>{if(!ok)throw new Error('Stone fish pool R6: '+message);};
...
export function createXieqiquStoneFishPoolR6FromBase({baseOwner,signal}={}){
  ...
  try{
    ...
    for(const mount of baseOwner.context.mounts){
      ...
      let core=coreByBottom.get(key);if(!core){core=createStoneFishCarvedWaveR6({contactGeometry:baseOwner.seat.geometry,contactInfo:baseOwner.seat.diagnostics,bottomY,waterY,bodyMinimumY,worldScale:mount.placement.size});coreByBottom.set(key,core);geometryOwners.set(core.geometry,core);}
    ...
    group.name=stoneFishPoolR6StudyId+'-study';group.userData={...group.userData,assetId:stoneFishPoolR6StudyId,visualAcceptance:false,historicalCarvingVerified:false};group.updateMatrixWorld(true);signal?.throwIfAborted();
    function validate(){
  ...
  }catch(error){try{dispose();}catch(cleanup){throw new AggregateError([error,cleanup],'Stone fish pool R6 construction and cleanup failed',{cause:error});}throw error;}
}

export function createXieqiquStoneFishPoolR6Study({pixels,signal}={}){
  signal?.throwIfAborted();const baseOwner=createXieqiquStoneFishPoolStudy({pixels,signal});return createXieqiquStoneFishPoolR6FromBase({baseOwner,signal});
}
