...
const check=(ok,message)=>{if(!ok)throw new Error('Stone fish wave R7: '+message);};
 ...
 * R7 changes only authored material below the exact cap and 10 mm collar.
 * It remains a modern artistic inference, never a measured historical carving. */
export function createStoneFishCarvedWaveR7({contactGeometry,contactInfo,bottomY,waterY,bodyMinimumY,worldScale=1,spacing=.008}={}){
  ...
  const profiles=specs.map(s=>profileDistanceGrid(waveProfile(bottomY,waterY-s.drop,s.scale))),rearLip=createRearLipProgress(specs[1].scale,waterY-specs[1].drop);
  ...
  const field=(x,y,z)=>{const core=coreDistance(x,y,z),forward=waveDistance(0,x,y,z),rear=waveDistance(1,x,y,z),returning=waveDistance(2,x,y,z),shaped=refinedRearLipDistance(x,y,z,rear,Math.min(core,forward,returning));let d=smoothMin(core,forward,.030);d=smoothMin(d,shaped,.030);return smoothMin(d,returning,.030);};
  ...
  const geometry=new THREE.BufferGeometry();geometry.name='stone-fish-r7-rounded-rear-wave-lip';geometry.setAttribute('position',new THREE.Float32BufferAttribute(positions,3));geometry.setAttribute('uv',new THREE.Float32BufferAttribute(uv,2));geometry.setIndex([...dry,...wet]);geometry.addGroup(0,dry.length,0);geometry.addGroup(dry.length,wet.length,1);geometry.computeVertexNormals();
  ...
  geometry.userData={body:'single-three-folded-wave-support',supportVariant:'r7',historicalCarvingVerified:false,originalContactCapRetained:true,roofVertices:n,roofTriangles:roofCount/3,bottomY,waterY,worldScale,waveCount:waves.length,maximumWaveY,broadenedSubmergedFoot:false,spacing,diagonalRepairs,fieldGrid:[nx,ny,nz],distanceGridBytes:profiles.reduce((sum,p)=>sum+p.bytes,0)+values.byteLength+rearLip.bytes,authoredRevision:'rear-lip-side-taper-rounded-section-r7-second',rearLip:{wave:specs[1].id,widthFactors:[1,.90,.65],roundingK:.070*specs[1].scale,minimumProtectedAir:2*spacing,fullAirGate:.060,upperFade:[.260,.290],protectedFusionGap:.030,fullDominanceGap:.060,stations:rearLip.stations,scalarGridBytes:rearLip.bytes},transitionContinuity:'zero radial slope at original vertical collar and vertical field bearing',transitionRows,cornerRoundness:{maximumWeight:Math.max(...cornerWeights),affectedBoundaryVertices:cornerWeights.filter(w=>w>0).length,maximumTravelGain:.16},capCollar:{start:collarStart,count:collar.length,depth:.010,sourceBoundary:upper.slice()},normalSource:'area-weighted shared Float32 surface normals; unchanged cap normals'};
  let disposed=false;
  const validate=()=>{
