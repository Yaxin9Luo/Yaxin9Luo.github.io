...
function venue(item, lang = 'en') {
  return `<span class="paper-venue${item.venue === 'arXiv' ? ' paper-venue-preprint' : ''}">${escape(item.kind==='software'?copy(lang,'Open source','开源项目'):item.venue)} <span>${escape(item.year)}</span></span>`;
}
...
function primaryLinks(lang) {
  return `<div class="journal-actions">${anchor(cvForLanguage(lang), copy(lang, 'Read my CV', '阅读简历'), 'journal-button journal-button-primary', 'file')}${anchor(links.scholar, 'Google Scholar', 'journal-button journal-button-outline', 'book')}${anchor(links.github, 'GitHub', 'journal-button journal-button-outline')}</div>`;
}
...
function journeyView(lang) {
  ...
  <div class="journal-next"><div><p>${escape(copy(lang, 'The formal version, all in one place.', '正式简历，完整呈现。'))}</p></div>${anchor(cvForLanguage(lang), copy(lang, 'Open CV · PDF', '打开简历 · PDF'), 'journal-button journal-button-primary', 'file')}</div>`;
}
...
function contactView(lang) {
  ...
  <div class="journal-cv-note"><div>${icon('file')}<div><h3>${escape(copy(lang, 'Curriculum vitae', '个人简历'))}</h3><p>${escape(copy(lang, 'Education, research, and experience in a downloadable PDF.', '可下载的 PDF，包含教育、研究与工作经历。'))}</p></div></div>${anchor(cvForLanguage(lang), copy(lang, 'Open CV', '打开简历'), 'journal-button journal-button-primary', 'arrow')}</div>`;
}
...
function entityHeader(item,lang,kind){
  return `<nav class="entity-breadcrumb" aria-label="${escape(copy(lang,'Breadcrumb','路径'))}">${sectionButton(kind==='paper'?'publications':'projects',copy(lang,kind==='paper'?'All publications':'All projects',kind==='paper'?'全部论文':'全部项目'),'journal-text-button')}<button class="journal-text-button" data-action="share-content">${escape(copy(lang,'Copy link','复制链接'))}${icon('external')}</button></nav><header class="entity-heading">${venue(item,lang)}<h2 id="entity-title" tabindex="-1">${escape(local(item.title,lang))}</h2>${lang==='zh'?`<p class="entity-original" lang="en">${escape(item.title.en)}</p>`:''}<div class="entity-facts"><span>Yaxin Luo</span><span>${escape(local(publicationRole(item),lang))}</span><span>${escape(item.kind==='software'?copy(lang,'Open-source project','开源项目'):item.venue==='arXiv'?copy(lang,'Preprint','预印本'):copy(lang,'Published paper','已发表论文'))}</span></div></header>`;
}
