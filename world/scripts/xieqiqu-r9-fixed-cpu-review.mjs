...
export async function inspectSculptureCPU(owner,destination,{constructionSeconds=null,viewSpecs,size=2048,fixedBounds=null}={}){
  ...
  for(const [view,spec]of Object.entries(viewSpecs)){
    ...
    const box=fixedBounds?new THREE.Box3(new THREE.Vector3(...fixedBounds.min),new THREE.Vector3(...fixedBounds.max)):new THREE.Box3().setFromObject(owner.group),extent=box.getSize(new THREE.Vector3()),minimum=box.min.clone();
