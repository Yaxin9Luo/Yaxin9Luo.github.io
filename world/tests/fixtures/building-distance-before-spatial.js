// Frozen pre-spatial runtime oracle. Original SHA-256 bc2feb0f0a5108c1ad51e80b7287d00b90ebb569a5f715d346156df70540a142; only relative imports relocated.
import * as THREE from 'three';
import {overviewAffineScaleBound} from '../../src/yuanmingyuan/asset-overview.js';
...
export async function createBuildingDistanceAsset(source,{id,sourceArchiveDigest,maximumErrorWorld,candidateProvider,candidateError,sparseInstanceLimit=32,maximumMergedVertices=750000,signal,onProgress,yieldControl=yieldTask}={}){
  if(!source?.group?.isObject3D||source.group.parent||!Number.isFinite(maximumErrorWorld)||maximumErrorWorld<=0||!Number.isInteger(sparseInstanceLimit)||sparseInstanceLimit<0||!Number.isInteger(maximumMergedVertices)||maximumMergedVertices<3||typeof candidateProvider!=='function'||typeof candidateError!=='function')throw new Error('A standalone static source, explicit world error and candidate provider are required.');
  ...
  const owned=new Set(),candidates=new Map(),buckets=new Map(),sourceBounds=new THREE.Box3(),outputMaterials=new Map(),groupOrders=new Map(),geometryRecords=new Map(),geometryBoxes=new Map();
  ...
  const dispose=()=>{if(disposed)return disposal;disposed=true;const counts={geometries:0,materials:0,instancedMeshes:0};for(const resource of owned){if(resource.isBufferGeometry)counts.geometries++;else if(resource.isMaterial)counts.materials++;else if(resource.isInstancedMesh)counts.instancedMeshes++;resource.dispose();}owned.clear();candidates.clear();buckets.clear();geometryRecords.clear();geometryBoxes.clear();outputMaterials.clear();groupOrders.clear();group.clear();disposal={...counts,borrowedSourceResourcesDisposed:0,idempotent:true};return disposal;};
  ...
  try{
    ...
    for(let ordinal=0;ordinal<nodes.length;ordinal++){
      ...
      for(let instance=0;instance<count;instance++){
        ...
        const world=new THREE.Matrix4().multiplyMatrices(originalRoot,local);sourceBounds.union(sourceBox.clone().applyMatrix4(world));stats.instances++;
        ...
        const key=[chosen.uuid,material.uuid,node.customDepthMaterial?.uuid??'',node.customDistanceMaterial?.uuid??'',JSON.stringify(f)].join('|');
        if(!buckets.has(key))buckets.set(key,{geometry:chosen,material,flags:f,depth:node.customDepthMaterial,distance:node.customDistanceMaterial,records:[]});
    ...
    for(const bucket of buckets.values()){
      ...
      if(merge){
        const key=[material.uuid,JSON.stringify(f),layout.key].join('|');if(!mergeBuckets.has(key))mergeBuckets.set(key,{material,flags:f,layout,items:[],vertices:0});
        let destination=mergeBuckets.get(key);
        if(destination.vertices+geometry.attributes.position.count*records.length>maximumMergedVertices){buildMerged(destination);destination={material,flags:f,layout,items:[],vertices:0};mergeBuckets.set(key,destination);}
        ...
        mesh.userData={representation:'distance',sourceBindings:bindingRuns(records)};attach(mesh,f);report.instancedDraws++;
      }
    }
    ...
    function buildMerged(bucket){
      ...
      const mesh=new THREE.Mesh(geometry,outputMaterials.get(key));mesh.name=geometry.name;mesh.userData={representation:'distance',sourceBindings:bindings};attach(mesh,bucket.flags);report.mergedDraws++;
    }
