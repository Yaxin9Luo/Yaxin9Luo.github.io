...
function carvedRobe() {
  const rows = [
    [.145,.46,.34,.06], [.18,.65,.46,.08], [.28,.72,.50,.10],
    [.40,.69,.52,.10], [.51,.60,.45,.075], [.63,.49,.35,.035],
    [.78,.52,.31,.005], [.94,.54,.275,-.025], [1.09,.50,.25,-.045],
    [1.23,.43,.225,-.060], [1.35,.33,.185,-.074], [1.44,.21,.137,-.080],
    [1.51,.135,.104,-.080], [1.555,.10,.080,-.080],
  ];
  const rings = [[[0,.140,.06]]], sides = 192, levels = 176;
  for (let i = 0; i <= levels; i++) {
    const y = THREE.MathUtils.lerp(.145, 1.555, i / levels), [rx, rz, cz] = profileAt(rows, y);
    rings.push(Array.from({ length: sides }, (_, j) => {
      const angle = j * TAU / sides, s = Math.sin(angle), c = Math.cos(angle), x = rx*s;
      const face = Math.max(0,c) ** 2, rear = Math.max(0,-c) ** 2;
      let z = cz + rz*c;
      const collarT = (1.53-y)/.93;
      const left = -.115 - .15*Math.sin(collarT*Math.PI) + .21*collarT;
      const right = .125 + .15*Math.sin(collarT*Math.PI) - .39*collarT;
      const collar = gate(y,.64,1.56,.05);
      z += face*collar*(.029*gaussian(x-left,.030) - .025*gaussian(x-left-.042,.025) + .024*gaussian(x-right,.024) - .018*gaussian(x-right+.035,.021));
      // Broad sleeve fall continues from the shoulder through the elbow into the lap.
      const arm = .27 + .19*Math.sin((y-.64)/.76*Math.PI);
      z -= face*gate(y,.60,1.36)*.038*gaussian(Math.abs(x)-arm,.045);
      for (let fold = 0; fold < 3; fold++) {
        const sleeveY = .71 + fold*.13 + .58*(Math.abs(x)-.30)**2;
        z += face*gaussian(Math.abs(x)-.40,.18)*(.022*gaussian(y-sleeveY,.033)-.023*gaussian(y-sleeveY-.035,.024));
      }
      // Two overlapping robe-covered legs, expressed as continuous folded stone.
      const lap = gate(y,.18,.66,.055);
      z += face*lap*(.039*gaussian(y-(.435+.22*x),.049)-.057*gaussian(y-(.485+.22*x),.035));
      z += face*lap*(.028*gaussian(y-(.275-.16*x),.033)-.040*gaussian(y-(.319-.16*x),.027));
      z += face*lap*.012*Math.sin(17*x+7*y)*gaussian(y-.25,.16);
      const hem = 1-THREE.MathUtils.smoothstep(y,.33,.78);
      z += c*hem*(.014*Math.sin(angle*11+1.7*y)+.007*Math.sin(angle*23-3*y));
      z += rear*gate(y,.24,1.35)*.009*Math.sin(angle*13+2.8*y);
      return [x,y,z];
    }));
  }
...
function restingHand(side) {
  const rings = [], count = 44;
  rings.push([[side*.295,.806,.351]]);
  for (let i = 0; i <= 42; i++) {
    const t = i/42, width = .069*Math.sin(Math.PI*(.08+.86*t))*(1-.20*t);
    const cx = side*(.295-.11*t), cy = .798-.115*t, cz = .363+.142*t;
    rings.push(Array.from({length:count},(_,j)=>{
      const a=j*TAU/count;
      const grooves=.0035*Math.cos(a*4+1.4*t)*Math.sin(Math.PI*t);
      return [cx+Math.cos(a)*width,cy+Math.sin(a)*(.031+grooves),cz];
    }));
  }
  rings.push([[side*.185,.681,.507]]);
  return ringSurface(rings,'resting-hand-with-knuckle-contours');
}

export function seatedBody(b, figure, id) {
  const body = namedGroup(figure,`zodiac-${id}-body-stone`,{pose:'seated robed human body',evidence:'engraving-supported seated pose; continuous robe, folds, hands and unseen back are authored interpretations',construction:'continuous indexed carved robe surface'});
  b.box(body,b.m.stone,0,.072,.08,1.47,.144,1.22,.045);
  b.add(body,b.prototype('haiyantang-r3-carved-robe',carvedRobe),b.m.stone);
  for (const side of [-1,1]) b.add(body,b.prototype(`haiyantang-r3-resting-hand-${side}`,()=>restingHand(side)),b.m.stone);
}

const RAT = {
  id:'rat', mouth:[.034,.024,-.165,.482], eye:[.125,.43,.030,.032],
  profile:[[-.265,.012,.025,.015],[-.225,.17,.23,.025],[-.11,.245,.32,.035],[.035,.259,.327,.004],[.18,.224,.262,-.047],[.31,.154,.172,-.101],[.405,.097,.108,-.126],[.466,.082,.084,-.132]],
  ears:[{root:[.216,.188,-.052],tip:[.293,.508,-.005],width:.085,cup:.042}],
  nose:[.048,.023,-.102,.482],
};

function headPoint(spec, z, angle) {
  const [rx,ry,cy]=profileAt(spec.profile,z);
  const grain=.00055*(Math.sin(angle*109+z*243)+.35*Math.sin(angle*197-z*361));
  return V(Math.cos(angle)*(rx+grain),cy+Math.sin(angle)*(ry+grain),z);
}
...
function continuousHead(spec) {
  ...
  for(let i=1;i<=18;i++) {
    ...
    rings.push(Array.from({length:n},(_,j)=>{
      ...
      return [THREE.MathUtils.lerp(p[0],Math.cos(a)*rx,s),THREE.MathUtils.lerp(p[1],my+Math.sin(a)*ry,s),THREE.MathUtils.lerp(p[2],mz,t)+.012*Math.sin(t*Math.PI)];
    }));
  }
  for(let i=1;i<=16;i++) {
    const t=i/16;
...
function cuppedEar(ear,side) {
  ...
  for(const back of [false,true]) for(let i=0;i<=lengthSteps;i++) for(let j=0;j<=widthSteps;j++) {
    ...
    const z=THREE.MathUtils.lerp(ear.root[2],ear.tip[2],t)+ear.cup*(u*u-.72)*Math.sin(Math.PI*t)+(back?-.014:0);
...
function eyeDetail(b,head,spec,side) {
  ...
  b.add(head,lensPatch(rx,ry,.012,`${spec.id}-inset-ocular-surface`),b.m.copper,p.toArray(),undefined,q,true);
  const rim=[];
  for(let i=0;i<=72;i++) {
    const a=i*TAU/72;
    rim.push(V(Math.cos(a)*(rx+.004),Math.sin(a)*(ry+.004),.004).applyQuaternion(q).add(p));
  }
  b.tube(head,rim,.0034,b.m.copper,72);
}
